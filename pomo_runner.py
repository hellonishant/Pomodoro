...
if path.isfile(STATE_FILE):
    print("Pomodoro session is already running, exiting")
    sys.exit()
...
try:
    ...
    def notify_user(title, message):
        if ENABLE_DESKTOP_NOTIFS:
            try:
                call(['notify-send', title, message, '-t', '10000'])
            except FileNotFoundError:
                print(
                    "Skipping desktop notification because `notify-send` wasn't recognized."
                )
    ...
    if args.interactive:
        ...
        try:
            pomo_state = PomodoroState(ACTIVE_STAGE_MINUTES, REST_STAGE_MINUTES,
                                    STATE_FILE)
            while True:
                ...
                notify_user("Pomodoro #%d completed" % pomo_state.pomos_completed,
                            "Time for the rest stage")
                print("Time for the rest stage.")
                run_stage(Stage.REST, progress_callback=update_progress_line)
                pomo_state.prep_for_active()

                notify_user(
                    "Rest stage #%d completed" % pomo_state.pomos_completed,
                    "Go to the CLI to start pomodoro #%d" %
                    (pomo_state.pomos_completed + 1))
                print("Rest stage finished - You've completed %d pomodoro(s)." %
                    pomo_state.pomos_completed)
        except KeyboardInterrupt:
            pomo_state.kill()
        print("Finished - You completed a total of %d pomodoro(s)." %
            pomo_state.pomos_completed)
    else:
        ...
        print("Finished - You completed a total of %d pomodoro(s)." %
            pomo_state.pomos_completed)
        notify_user(
            "Pomodoro Session Finished", "You completed a total of %d pomodoro(s)."
            % pomo_state.pomos_completed)
finally:
    unlink(STATE_FILE)
